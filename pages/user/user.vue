<template>
	<view class="content">
		...
		<div class="uni-inline-item">
			<span>车架号</span>
			<input type="text" class="item_input" v-model="frameNo" @blur="watchInput" placeholder-class="place-holder"
			 placeholder="请输入车架号" @focus="focusInput">
			<img src="../../static/img/delete_icon.png" class="close" v-if="frameClose" @click="closeFrame">
		</div>

		<div class="uni-inline-item">
			<span>发动机号</span>
			<input type="text" class="item_input" v-model="engineNo" @blur="watchEngineInput" placeholder-class="place-holder"
			 placeholder="请输入发动机号" @focus="focusEngineInput">
		...
		<div class="uni-inline-item">
			<span>品牌型号</span>
			<input type="text" class="item_input" v-model="brandCode" @blur="watchBrandInput" placeholder-class="place-holder"
			 placeholder="请输入品牌型号">
		</div>

		<div class="uni-inline-item">
			<span>车主姓名</span>
			<input type="text" v-model="name" placeholder="请输入车主姓名" @blur="fixIos" placeholder-class="place-holder" class="item_input">
		</div>

		<div class="uni-inline-item">
			<span>身份证号</span>
			<input type="text" v-model="idcard" placeholder="请输入证件号码" @blur="watchCardInput" placeholder-class="place-holder"
			 class="item_input">
		</div>

		<div class="uni-inline-item">
			<span>手机号码</span>
			<input type="number" v-model="mobile" placeholder="请输入手机号码" @blur="fixIos" placeholder-class="place-holder" class="item_input">
		</div>
		...
		<view class="btn_wrap" :class="{'hideBtnClass': hideBtn}">
			<button type="primary" class="primary" hover-class="button-hover" @tap="basic">下一步</button>
		...
		<img src="../../static/img/close.png" class="close_icon" v-if="isShowModal" :class="{'close_show_icon':!isShowBtn}"
		 @tap="closeModal">
...
<script>
	...
	export default {
		...
		onLoad() {
			...
			this.license_no = this.$root.$mp.query.license_no ? this.$root.$mp.query.license_no : '陕AT9V26';
			this.id = this.$root.$mp.query.id ? this.$root.$mp.query.id : '489';
			this.getVechileData();
			this.isShowBtn = true;
		...
		methods: {
			...
			async getVechileData() {
				let that = this;
				let res = await getVechileData({
					license_no: that.license_no
				});
				if (res.code == 200) {
