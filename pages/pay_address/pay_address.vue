      ...
      <span class="item_right">200元</span>
		...
		<div class="wrap">
			<div class="item_wrap">
				...
				<input class="uni-input item_right" v-model="name" placeholder="请输入姓名" @blur="fixIos" />
			</div>
			<div class="item_wrap">
				<span class="item_text">手机号码：</span>
				<input class="uni-input item_right" v-model="mobile" placeholder="请输入手机号" @blur="fixIos" />
			</div>
	...
	export default {
		...
		methods: {
			...
			next() {
				this.showModal = false;
				uni.navigateTo({
					url: "../pay_one/pay_one?jiaoqiang_order_id=" + this.jiaoqiang_order_id+"&quotation_id="+this.quotation_id
				})
			},
