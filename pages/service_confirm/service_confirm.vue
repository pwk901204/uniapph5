<template>
	<div class="contain">
		...
		<div class="license">
			...
			<navigator :url="'/pages/agreement/agreement?monthly_expense='+monthly_expense+'&mileage_expense='+mileage_expense+'&quotation_id='+quotation_id">
		...
		methods: {
			...
			next: function() {
				...
				if (!openid) {
					if (isWeiXin()) {
						...
						// alert(encodeURIComponent(location.origin))
						window.location.href =
							"https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx49aad3a138063b53&redirect_uri=https://api.kaikaibao.com.cn/3.1/getoauth?redit_url=" +
							encodeURIComponent(location.origin) +
							"%2flccx2%2findex.html%23%2fpages%2fpay_address%2fpay_address%3fquotation_id%3d" + this.quotation_id +
							"%26partner_id%3d" + partner_id + "%26imei%3d" + imei +
			...
			async getQuotations() {
				...
				if (res.code == 200) {
					...
					if (this.item.vin) {
						this.trueVin = this.item.vin.substring(0, 3) + "***********" + this.item.vin.substring(this.item.vin.length - 3,
							this.item.vin.length);
					}

					if (this.item.engine_no) {
						this.trueEngineNo = this.item.engine_no.substring(0, 1) + "****" + this.item.engine_no.substring(this.item.engine_no
							.length - 1, this.item.engine_no.length);
					}
